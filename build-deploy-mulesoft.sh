...
if [[ "${SCRIPT_ENVIRONMENT}" == "LHS" ]]; then
        ...
        git config --global http.proxy ${MULE_PROXY_HOST}:${MULE_PROXY_HOST_PORT}
    		...
    		if [[ "${VERSION}" != *"SNAPSHOT"* ]]; then
    			codeToCheckoutFrom=${ReleasePrefixOrTagName}_${VERSION}
    		fi
...
case ${ENVIRONMENT} in
    ...
    dev-saas-proc)
    echo 'Starting deploying to dev-sas-proc ...'
    ...
    sit-saas-proc)
    echo 'Starting deploying to SIT ...'
    ...
    uat1-saas-proc)
    echo 'Starting deploying to UAT1 ...'
    ...
    uat2-saas-proc)
    echo 'Starting deploying to UAT2 ...'
    ...
    ppt-saas-proc)
    echo 'Starting deploying to PPT ...'
...
echo "==============================UPLOAD TO NEXUS STARTED====================================="
echo "mvn clean package -U deploy:deploy -DskipMunitTests=true -Dhttps.proxyHost=${MULE_PROXY_HOST}  \
        ...
        -DdistributionManagement.repository.snapshotURL=${NEXUS_SNAPSHOT_URL} -X"
mvn clean package -U deploy:deploy -DskipMunitTests=true -Dhttps.proxyHost=${MULE_PROXY_HOST}  \
        -Dhttps.proxyPort=${MULE_PROXY_HOST_PORT} \
        -Dmule.key=${MULE_KEY} \
        -DdistributionManagement.releaseRepository.repoId=${NEXUS_RELEASE_REPO_ID} \
        -DdistributionManagement.snapshotRepository.repoId=${NEXUS_SNAPSHOT_REPO_ID} \
        -DdistributionManagement.repository.releaseURL=${NEXUS_RELEASE_URL} \
        -DdistributionManagement.repository.snapshotURL=${NEXUS_SNAPSHOT_URL}

if [[ "$?" -ne 0 ]] ; then
    echo 'Error with Build/Deploy package into Nexus!!!'
    exit 1
fi
